...
async function claimBiosample() {
  ...
  const signature = await wallet.signMessage(dataArray);
    ...
    const response = await fetch(`https://app.genobank.io/claim/${tokenID.substring(2)}`, {
        method: 'POST',
        headers: {
            ...
            async function () {
              ...
              $('#tx').html(
                ` <h3>Success! Kit is Activated</h3>
                  ...
                  <a id="txLink" href="https://rinkeby.etherscan.io/address/${generateAddress}" target="_blank">    
